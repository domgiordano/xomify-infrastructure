...
# Dynamically create endpoints for wrapped lambdas
module "wrapped_endpoints" {
  ...
  uri                     = aws_lambda_function.wrapped[each.key].invoke_arn
...
resource "aws_lambda_permission" "wrapped_data_permission" {
  for_each      = { for lambda in local.wrapped_lambdas : lambda.name => lambda }
  statement_id  = "AllowWrapped${title(each.value.name)}Api"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.wrapped[each.value.name].function_name
