...
# Dynamically create endpoints for user lambdas
module "user_endpoints" {
  ...
  uri                     = aws_lambda_function.user[each.key].invoke_arn
...
resource "aws_lambda_permission" "user_data_permission" {
  for_each      = { for lambda in local.user_lambdas : lambda.name => lambda }
  statement_id  = "AllowUser${title(each.value.name)}Api"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.user[each.value.name].function_name
